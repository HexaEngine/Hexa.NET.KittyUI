...
{
    using Hexa.NET.KittyUI.Security.Cryptography;
    using System;
    ...
    /// <summary>
    /// A thread-safe persistent memory cache with lazy disk writing.
    /// </summary>
    public class WebCache : IDisposable
    {
        ...
        private readonly Dictionary<uint, WebCacheEntry> hashTable = [];
        ...
        private bool TryGetEntry(string key, [NotNullWhen(true)] out WebCacheEntry? entry)
        {
            ...
            bool result = hashTable.TryGetValue(hash, out entry);
            return result && (!result || !entry!.PersistenceState.IsGhost);
        }

        private bool TryGetEntry(uint key, [NotNullWhen(true)] out WebCacheEntry? entry, bool allowGhosts = false)
        {
            bool result = hashTable.TryGetValue(key, out entry);
            return result && (!result || (allowGhosts || !entry!.PersistenceState.IsGhost));
        }
        ...
        private void ReadIndexFile()
        {
            ...
            {
                var entry = WebCacheEntry.Read(fs, buffer);
                entries.Add(entry);
