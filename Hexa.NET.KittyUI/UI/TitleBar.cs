namespace Hexa.NET.KittyUI.UI
{
    ...
    public unsafe class TitleBar : ITitleBar
    {
        ...
        {
            builder.AddTitle();
            builder.AddButton($"{MaterialIcons.Menu}", 0x1CCCCCCC, 0x1CCCCCCC, this, userdata =>
            {
                if (userdata is TitleBar titleBar)
                {
                    titleBar.Navigation!.ShowMenu();
                }
            }, userdata => userdata is TitleBar { Navigation: not null });

            builder.AddButton($"{MaterialIcons.ArrowBack}", 0x1CCCCCCC, 0x1CCCCCCC, this, userdata =>
            {
                if (userdata is TitleBar titleBar)
                {
                    titleBar.Navigation!.NavigateBack();
                }
            }, userdata => userdata is TitleBar { Navigation: not null });
